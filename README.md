...
把散落的 JD、简历和面经，变成一套有来源、能训练、可复盘的面试准备流程。

![面试工作台产品总览](./docs/assets/showcase/showcase-overview.jpg)

这是一个本地优先的单用户工具。没有 DeepSeek Key 或 OpenCLI，也能导入材料、生成规则题库、逐题训练和完整备份恢复。

## 30 秒看懂

**目标岗位与材料 → 来源化题库 → 单题作答 → 证据审阅 → 最多一次追问**

![面试工作台完整流程动图](./docs/assets/showcase/product-tour.gif)

工作台不会只给出一串泛化题目。每道题都会标明来自 JD、简历还是具体面经；回答后，审阅结果会区分事实、推断和待补充信息，帮助你找到下一次回答最值得改进的地方。

## 四个核心步骤

| 1. 准备岗位与材料 | 2. 生成来源化题库 |
| --- | --- |
| ![填写目标岗位并整理 JD、简历和面经](./docs/assets/showcase/01-materials.jpg) | ![按来源查看 JD、简历和面经题目](./docs/assets/showcase/02-question-bank.jpg) |
| 按“公司＋岗位”保存材料，切换目标时互不混用。 | 面经原题保留来源，规则题和 AI 衍生题不会冒充真题。 |

| 3. 审阅回答证据 | 4. 完成一次针对性追问 |
| --- | --- |
| ![查看回答中的证据、推断和缺口](./docs/assets/showcase/03-evidence-review.jpg) | ![根据优先改进点完成唯一一次追问](./docs/assets/showcase/04-follow-up.jpg) |
| 可选用 DeepSeek 检查回答中的证据、事实缺口和表达问题。 | 首轮后最多追问一次，第二轮结束后自动封闭并保存。 |
